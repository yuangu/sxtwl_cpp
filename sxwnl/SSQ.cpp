...
int  SSQ::calc(long double jd, QSType qs)
{
	...
	if (jd < f1 || jd >= f3)
	{
		...
		if (qs == QType)
		{
			return (int)(qi_high((int)((jd + pc - 2451259) / 365.2422 * 24) * PI / 12) + 0.5); //2451259是1999.3.21,太阳视黄经为0,春分.定气计算
		}
		else
		{
			return (int)(so_high((int)((jd + pc - 2451551) / 29.5306) * PI * 2) + 0.5); //2451551是2000.1.7的那个朔日,黄经差为0.定朔计算
		}
	}
	...
	if (jd >= f2 && jd < f3) { //定气或定朔
		if (qs == QType) {
			D = (int)(qi_low((int)((jd + pc - 2451259) / 365.2422 * 24) * PI / 12) + 0.5); //2451259是1999.3.21,太阳视黄经为0,春分.定气计算
			n = QB.substr((int)((jd - f2) / 365.2422 * 24), 1); //找定气修正值
		}
		else {
			D = (int)(so_low((int)((jd + pc - 2451551) / 29.5306) * PI * 2) + 0.5); //2451551是2000.1.7的那个朔日,黄经差为0.定朔计算
			n = SB.substr((int)((jd - f2) / 29.5306), 1); //找定朔修正值
		}
		if (n == "1") return D + 1;
		if (n == "2") return D - 1;
		return D;
...
void SSQ::calcY(int jd) { //农历排月序计算,可定出农历,有效范围：两个冬至之间(冬至一 <= d < 冬至二)
	...
	if (YY >= -721 && YY <= -104) {
		...
		for (i = 0; i < 3; i++) {
			yy = YY + i - 1;
			//颁行历年首, 闰月名称, 月建
			if (yy >= -721) ns[i] = calc(1457698 - J2000 + int2(0.342 + (yy + 721)*12.368422)*29.5306, SType), ns[i + 3] = '十三', ns[i + 6] = 2;  //春秋历,ly为-722.12.17
			if (yy >= -479) ns[i] = calc(1546083 - J2000 + int2(0.500 + (yy + 479)*12.368422)*29.5306, SType), ns[i + 3] = '十三', ns[i + 6] = 2;  //战国历,ly为-480.12.11
			if (yy >= -220) ns[i] = calc(1640641 - J2000 + int2(0.866 + (yy + 220)*12.369000)*29.5306, SType), ns[i + 3] = '后九', ns[i + 6] = 11; //秦汉历,ly为-221.10.31
		}
